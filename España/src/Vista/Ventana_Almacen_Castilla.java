 ...
 */
	...
	public void colocarScroll() {
		...
		while(it.hasNext()) {
			int id;
			id=(int) it.next();
			mercancias[posicion]=(id)+" "+this.control.getEspana().getCastilla().getMercancia().get(id).getNombre()+" cantidad "+this.control.getEspana().getCastilla().getMercancia().get(id).getTotalkg()+"kg";
			posicion++;
